...
La solución propuesta se basa en una arquitectura de API \textit{REST ful}, que actúa como un puente de comunicación estandarizado, permitiendo que \textit{Jub} invoque los servicios de procesamiento avanzado de \textit{Nez} de manera transparente y eficiente. El \textit{middleware} implementado no solo facilita el intercambio de datos y procesos en tiempo real, sino que también establece las bases para la creación de una malla de servicios de ciencia de datos unificada, gestionando las operaciones de almacenamiento a través del cliente \textit{MictlanX}. 
