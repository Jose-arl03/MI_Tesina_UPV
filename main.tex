...
\newcommand{\Matricula}           {2130207}  %SU MATRICULA
...
\newcommand{\fechacarta}{4 de Diciembre de 2025}
\newcommand{\ncuatrimestre}{Septiembre-Diciembre 2025}
\newcommand{\nevalador}{Dr. Hiram Herrera Rivas}
\newcommand{\FechaExposicion}{08 de Diciembre de 2025}
\newcommand{\HoraExposionFormatoVenticuatroHoras}{12:00}
...
\newcommand{\NombreProyectoheader} {Diseño e implementación de un middleware para \\ la interoperabilidad entre la plataforma Nez \\ y el concentrador de servicios Jub}
\newcommand{\nasesorempresaria}   {DR. JOSÉ LUIS GONZÁLEZ COMPEÁN}
\newcommand{\fechaPortada}               {Diciembre de 2025}
...
Quiero expresar mi m\'as profundo agradecimiento a mi familia, quienes fueron el pilar fundamental para culminar mis estudios. Su apoyo incondicional, su confianza en m\'i y la paciencia que me brindaron en cada etapa de este proceso fueron esenciales para continuar avanzando incluso en los momentos m\'as dif\'iciles.

A mi madre, agradezco cada palabra de aliento, cada gesto de apoyo y cada sacrificio que hizo para que yo pudiera concentrarme y continuar con mi formaci\'on. Su fuerza y dedicaci\'on fueron una gu\'ia constante que me inspir\'o a no rendirme.

A mis hermanos, les agradezco por orientarme cuando me encontraba perdido, por ayudarme a comprender aquello que no entend\'ia y por acompa\~narme en los momentos de incertidumbre. Su disposici\'on y compa\~n\'ia hicieron que este camino fuera m\'as ligero.

A mis verdaderos amigos, gracias por estar presentes sin esperar nada a cambio, por escucharme, motivarme y ofrecerme su apoyo sincero. Su amistad fue un refugio en los d\'ias complicados y una fuente de energ\'ia para seguir adelante.

A mis compa\~neros, les agradezco por los buenos momentos compartidos, por el aprendizaje mutuo y por el compa\~nerismo que hizo m\'as llevadera esta etapa acad\'emica. Cada experiencia vivida junto a ustedes form\'o parte importante de este recorrido.
...
%\input{Resumen.tex}
El crecimiento de los sistemas de an\'alisis de datos en entornos cient\'ificos ha incrementado la necesidad de integrar plataformas heterog\'eneas mediante mecanismos de comunicaci\'on estandarizados. En el ecosistema desarrollado por el CINVESTAV Tamaulipas, la plataforma Nez genera productos de datos derivados de procesos de c\'omputo intensivo, mientras que MictlanX funge como un sistema de almacenamiento distribuido que alimenta a la plataforma Jub, utilizada como concentrador de informaci\'on para aplicaciones de investigaci\'on y monitoreo. Sin embargo, la ausencia de un mecanismo autom\'atico que conecte estos sistemas limita la construcci\'on de flujos de trabajo integrados.

Este trabajo presenta el dise\~no e implementaci\'on del \textit{Nez-Daemon Watcher}, un middleware de acoplamiento ligero que automatiza la interoperabilidad entre Nez y el ecosistema de almacenamiento de MictlanX. El sistema funciona como un servicio aut\'onomo en segundo plano, basado en un modelo reactivo orientado a eventos. Utiliza la monitorizaci\'on del sistema de archivos para detectar en tiempo real la creaci\'on de nuevos artefactos producidos por Nez y ejecuta un flujo de ingesta que incluye verificaci\'on de integridad, registro estructurado, c\'alculo de hash SHA-256 y carga al VSS de MictlanX mediante su cliente oficial.

La soluci\'on adopta principios de dise\~no modernos para sistemas distribuidos: concurrencia a trav\'es de trabajadores as\'incronos, resiliencia mediante reintentos con retroceso exponencial, arquitectura portable basada en contenedores Docker y configuraci\'on externa siguiendo las pr\'acticas de las aplicaciones de doce factores. El sistema tambi\'en incorpora mecanismos de cuarentena para archivos corruptos y extensiones para solicitudes de descarga mediante comunicaci\'on entre procesos usando sockets Unix.

La validaci\'on del prototipo se llev\'o a cabo mediante escenarios representativos de procesamiento de im\'agenes, demostrando un funcionamiento confiable del flujo de ingesta, un uso eficiente de recursos y una integraci\'on transparente con el almacenamiento distribuido. El middleware desarrollado constituye una base s\'olida para la futura integraci\'on con Jub y para la construcci\'on de mallas de servicios m\'as amplias dentro del ecosistema cient\'ifico institucional.

\textbf{Palabras clave:} Middleware, Interoperabilidad, Automatizaci\'on, Sistemas Distribuidos, Watchdog, MictlanX, Nez, Acoplamiento Ligero, Docker.
...
%\input{Summary.tex}
The rapid growth of data analysis systems in scientific environments has increased the need to integrate heterogeneous platforms through standardized communication mechanisms. Within the ecosystem developed at CINVESTAV Tamaulipas, the Nez platform generates data products derived from intensive computational processes, while MictlanX serves as a distributed storage system that supplies the Jub platform, used as a data hub for research and monitoring applications. However, the absence of an automatic mechanism linking these systems limits the construction of integrated workflows.

This work presents the design and implementation of the \textit{Nez-Daemon Watcher}, a lightweight coupling middleware that automates interoperability between Nez and the MictlanX storage ecosystem. The system operates as an autonomous background service, following a reactive event-driven model. It monitors the filesystem to detect new artifacts produced by Nez in real time and executes an ingestion workflow that includes integrity verification, structured logging, SHA-256 hash computation, and object upload to the MictlanX VSS using its official client.

The solution adopts modern design principles for distributed systems: concurrency through asynchronous workers, resilience via exponential backoff retry strategies, portability using Docker containers, and external configuration following twelve-factor application practices. The system also incorporates quarantine mechanisms for corrupted files and support for download requests through inter-process communication using Unix domain sockets.

The prototype was validated through representative image-processing scenarios, demonstrating reliable ingestion behavior, efficient resource usage, and transparent integration with the distributed storage system. The developed middleware provides a solid foundation for future integration with Jub and for building more extensive service meshes within the institutional scientific ecosystem.

\textbf{Keywords:} Middleware, Interoperability, Automation, Distributed Systems, Watchdog, MictlanX, Nez, Lightweight Coupling, Docker.
