...
El modelo \textbf{PuzzleMesh} es un marco conceptual desarrollado en el CINVESTAV para la construccion de estructuras de procesamiento de datos que, por diseno, son agnosticas a la infraestructura en la que se despliegan. Este modelo propone una abstraccion basada en la metafora de un "rompecabezas", con el objetivo de simplificar la manera en que se definen y ensamblan componentes dentro de sistemas distribuidos.

Bajo este enfoque, los elementos individuales del procesamiento pueden combinarse de forma modular, manteniendo independencia entre si y permitiendo que el flujo de datos se configure mediante reglas explicitas de composicion. Esto facilita la reutilizacion de componentes y la adaptacion del sistema a distintos entornos de ejecucion, en coherencia con los principios que caracterizan a las mallas de servicios \cite{sanchez2021efficient}.

El modelo proporciona asi una base formal para estructurar procesos complejos, favoreciendo la claridad en la definicion de responsabilidades, la organizacion del flujo de informacion y la capacidad de extender o modificar los elementos del sistema sin afectar la arquitectura global.
...
Un \textbf{Rompecabezas (R)} es una estructura de procesamiento de nivel superior que se forma al combinar un conjunto de piezas. La organizacion interna del rompecabezas se establece mediante un \textbf{Grafo Aciclico Dirigido (DAG)}, en el cual cada nodo corresponde a una pieza y cada arista representa una dependencia o un flujo de informacion entre ellas. Este enfoque permite definir el orden en el que deben ejecutarse las piezas y las condiciones bajo las cuales cada una puede iniciar su operacion.

En un rompecabezas, el DAG describe de manera explicita las relaciones entre las piezas, incluyendo aquellas que funcionan como origen de datos y las que actuan como destinos o etapas finales del procesamiento. De esta forma, un rompecabezas abarca un flujo de trabajo completo, desde una o varias fuentes hasta uno o varios puntos de salida, coordinando la ejecucion de las piezas involucradas segun las dependencias establecidas. Esto permite modelar procesos que requieren pasos secuenciales, tareas paralelas o combinaciones de ambas, manteniendo claridad sobre el recorrido que sigue la informacion en todo el sistema.

\subsubsection{El Metarompecabezas ($\Omega$): Interoperabilidad Inter-Sistemas}
...
El funcionamiento del middleware se estructura como un flujo de trabajo automatizado cuyo proposito es enlazar de manera transparente los procesos de la plataforma Nez con los servicios proporcionados por el ecosistema Jub. Este flujo depende del monitoreo constante del sistema de archivos y del intercambio controlado de artefactos mediante el cliente de MictlanX. A continuacion se describe con mayor detalle cada una de las etapas que conforman este proceso:

\begin{enumerate}
	\item El demonio del middleware se inicia como un servicio del sistema y entra en un estado de vigilancia pasiva. En este estado se mantiene ejecutando en segundo plano, sin intervenir activamente hasta que ocurre un evento de interes. Su consumo de recursos permanece minimo, lo cual permite que se ejecute de manera continua sin afectar el rendimiento general del entorno.
	
	\item El monitor \texttt{watchdog} se registra en el sistema operativo para observar el directorio de salida generado por la plataforma Nez. Este registro utiliza los mecanismos nativos del sistema para recibir alertas en cuanto se produzca una modificacion en dicho directorio, evitando la necesidad de realizar consultas periodicas o escaneos manuales.
	
	\item La plataforma Nez completa una de sus tareas internas de procesamiento y genera un nuevo producto de datos. Este producto puede consistir en un archivo individual, una carpeta con varios elementos o cualquier otro artefacto que forme parte del flujo de trabajo definido. Una vez generado, Nez lo deposita en el directorio previamente configurado.
	
	\item El sistema operativo detecta la creacion del nuevo archivo o carpeta y emite una notificacion dirigida al proceso \texttt{watchdog}. Esta notificacion se produce de forma inmediata, lo cual permite que el middleware reaccione sin retrasos adicionales, manteniendo la sincronizacion entre ambos sistemas.
	
	\item La notificacion recibida actua como el \textbf{disparador (trigger)} del flujo de interoperabilidad. A partir de este punto, la logica principal del middleware se activa y abandona temporalmente su estado pasivo para iniciar el procesamiento del nuevo artefacto.
	
	\item El middleware accede al archivo recien creado y analiza su contenido basico, asi como los metadatos asociados. Esta etapa permite identificar el tipo de producto, su origen dentro del procesamiento de Nez y cualquier informacion necesaria para su posterior registro en la plataforma MictlanX.
	
	\item Una vez identificado el producto, el middleware invoca al cliente de la plataforma MictlanX, al cual entrega el artefacto junto con la informacion necesaria para su registro. En este paso, el middleware actua explicitamente como \textbf{cliente de servicio}, delegando en MictlanX la gestion del almacenamiento, catalogacion o publicacion del producto dentro del ecosistema Jub. Esta invocacion puede incluir validaciones adicionales, reintentos o verificacion de estados segun lo requerido por el flujo operativo.
	
	\item Tras completar la operacion de envio, el middleware registra internamente el exito o fallo de la transferencia y procede a regresar a su estado inicial de espera. A partir de este punto queda preparado para recibir nuevas notificaciones del sistema operativo, permitiendo que el flujo de interoperabilidad se repita de manera continua y automatizada.
\end{enumerate}
