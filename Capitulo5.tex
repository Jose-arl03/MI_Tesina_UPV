...
\begin{itemize}
    ...
    \item \textbf{RF-04: Gestión de Cuarentena}. Para asegurar la alta disponibilidad, si un archivo falla repetidamente durante el proceso de carga, debe ser movido a un directorio de \texttt{Cuarentena}, aislando los archivos problemáticos sin detener el servicio.
...
\begin{itemize}
    \item \textbf{RNF-01: Concurrencia.} El sistema debe procesar múltiples archivos de forma concurrente mediante un grupo de \texttt{trabajadores asíncronos} para maximizar el rendimiento.
...
Cuando el Watcher necesita cargar un archivo, establece una comunicacion directa con el Router. Este componente funciona como el punto inicial del flujo de ingreso de datos dentro del ecosistema MictlanX. A partir de esta solicitud, el Router delega al SPM la tarea de determinar la estrategia adecuada para almacenar el archivo. El SPM realiza esta evaluacion considerando diversos criterios, tales como el nivel de carga actual de los Peers disponibles, el factor de replicacion solicitado y las politicas de distribucion configuradas en el sistema.

Con base en esta informacion, el SPM genera una decision que especifica en que nodos debe almacenarse el archivo y bajo que condiciones operativas debera llevarse a cabo la transferencia. Una vez emitida esta resolucion, el Router actua como intermediario y guia al cliente para ejecutar el proceso de envio hacia los Peers seleccionados. El Router no almacena los datos de manera directa; en cambio, coordina el flujo de informacion para que la transferencia se realice de manera ordenada y conforme a las directrices definidas por el SPM.

Esta arquitectura desacoplada distingue de forma clara entre el plano de control y el plano de datos. El SPM opera en el plano de control, gestionando los metadatos, la planificacion de las replicas y la toma de decisiones sobre la distribucion de los recursos. Por su parte, el Router y los Peers funcionan en el plano de datos, encargandose de las operaciones de transferencia, recepcion y almacenamiento fisico de los archivos. Esta separacion permite que el sistema mantenga independencia entre la logica de control y las operaciones intensivas de datos, lo que contribuye a la escalabilidad y flexibilidad generales de MictlanX.

El \textit{Nez-Daemon Watcher} cumple el rol de un cliente especializado que traduce eventos del sistema de archivos en operaciones complejas dentro del ecosistema distribuido. Su tarea principal es transformar una simple notificacion de creacion de archivo en una serie de interacciones estructuradas que culminan en la insercion del artefacto en la red de almacenamiento gestionada por MictlanX. Esta funcion permite integrar sistemas externos, como la plataforma Nez, con la infraestructura de almacenamiento sin requerir modificaciones en los procesos internos de dichas plataformas.

\subsection{Arquitectura del Sistema}

La arquitectura del \textit{Nez-Daemon Watcher} se presenta en esta seccion y se encuentra organizada para desempeñar el papel de un componente de borde (\textit{edge component}) dentro del ecosistema del VSS de MictlanX. El objetivo principal de esta arquitectura es proporcionar un mecanismo de vigilancia y transferencia de datos que funcione de manera autonoma, sin requerir intervencion manual y manteniendo una interaccion estable con los servicios externos.

El sistema se construye a partir de un conjunto de modulos claramente delimitados, cada uno con responsabilidades especificas. Esta organizacion modular permite separar las tareas de monitoreo del sistema de archivos, el manejo de eventos, la preparacion de artefactos y la comunicacion con el cliente de MictlanX. De este modo, es posible aislar errores, ajustar componentes individuales y mantener un comportamiento predecible dentro de distintos entornos de ejecucion.

Un elemento central de esta arquitectura es su enfoque orientado a eventos. El \textit{Watcher} no realiza escaneos periodicos ni consultas repetitivas sobre el sistema de archivos, sino que responde exclusivamente a las notificaciones del sistema operativo. Esto reduce el consumo de recursos y elimina cargas innecesarias en el equipo donde se despliega. Cuando se recibe un evento, el sistema activa su logica interna para identificar el archivo, procesarlo segun las reglas definidas y enviarlo al VSS.

La Figura \ref{fig:arquitectura_watcher} muestra la interaccion entre los modulos que componen el \textit{Nez-Daemon Watcher}. La ilustracion representa el flujo general del sistema, desde la recepcion de eventos por parte del monitor hasta la comunicacion final con la plataforma MictlanX, permitiendo visualizar de manera estructurada el papel de cada componente dentro del proceso completo.

\begin{figure}[h]
    \centering
    % Código fuente en Diagramas UML/arquitectura.puml
    \includegraphics[width=1.12\textwidth, height=11.5cm]{Diagramas UML/arquitectura.png}
...
\begin{figure}[h]
    ...
    \includegraphics[width=1.1\textwidth]{Diagramas UML/contexto.png}
...
\begin{figure}[h]
	...
	% Se define ancho Y alto.
	\includegraphics[width=1.02\textwidth, height=4.4cm]{Diagramas UML/casos_de_uso.png} 
...
\begin{figure}[h]
    ...
    \includegraphics[width=1.05\textwidth, height=10.9cm]{Diagramas UML/secuencia_carga.png}
...
\begin{itemize}
    \item \textbf{Comprobación de Salud Inicial:} Antes de iniciar el bucle principal, el watcher realiza una llamada de \texttt{health check} al Router de MictlanX. Si el servicio de almacenamiento no está disponible, el watcher termina con un error, evitando operar en un estado degradado.
...
El Algoritmo \ref{algo:worker} representa el bucle de eventos de cada trabajador, responsable de consumir tareas y despacharlas a la lógica apropiada.
